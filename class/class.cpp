...
public:
  ...
  int &operator[](int index) { return this->livedAges[index]; }
  ...
  bool operator==(const Human &other) {
    ...
    for (int i = 0; i < age; ++i) {
      if (this->livedAges[i] != other.livedAges[i])
        return false;
    }
  ...
  bool operator!=(const Human &other) { return !this->operator==(other); }
...
private:
  ...
  public:
    Seed(int mass) { this->mass = mass; }

    string GetInfo() { return "Seed: mass = " + to_string(mass); }
  };
...
public:
  ...
  static int GetCount() { return Apple::count; }
...
void editHuman(Human &human) { human.SetName("Andrey"); }

void Human::TakeApple(Apple &apple) {
  if (Apple::count > 0)
    cout << "Took an " << apple.color << " apple (weight: " << apple.weight
         << ", total count: " << Apple::count << ')' << endl;
  else
    cout << "No more apples" << endl;
}

void Human::EatApple(Apple &apple) {
  if (Apple::count > 0) {
    --Apple::count;
    cout << "Ate an " << apple.color << " apple (weight: " << apple.weight
         << ", total count: " << Apple::count << ')' << endl;
  } else
    cout << "No more apples left" << endl;
}
